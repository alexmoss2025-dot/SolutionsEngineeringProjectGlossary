...
function getCategoryCounts(terms: GlossaryTerm[]): Record<Category, number> {
  const ba = terms.filter((t) => t.category === 'Business Analyst').length
  const cs = terms.filter((t) => t.category === 'Company-Specific').length
  return { All: terms.length, 'Business Analyst': ba, 'Company-Specific': cs }
}
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ background: '#F8FAF9' }}>
      ...
      <header
        ...
        style={{ background: 'rgba(45, 106, 79, 0.97)', borderColor: 'rgba(255,255,255,0.1)' }}
      >
        <div className="max-w-[1200px] mx-auto px-4 sm:px-6 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs leading-tight" style={{ color: 'rgba(216, 243, 220, 0.8)' }}>
                Empower to create the exceptional
              </p>
            </div>
          </div>
      ...
      <section
        ...
        style={{ background: 'linear-gradient(180deg, #D8F3DC 0%, #F8FAF9 100%)' }}
      >
      ...
      <div className="max-w-[1200px] mx-auto w-full px-4 sm:px-6 mt-6 mb-4">
        <p className="text-sm font-medium" style={{ color: '#52B788' }}>
          Showing {filtered.length} of {allCounts.All} terms
        </p>
      </div>
        ...
        <AnimatePresence mode="popLayout">
          {filtered.length > 0 ? (
            <motion.div
              ...
              style={{ boxShadow: 'var(--shadow-sm)', border: '1px solid #e8f0ec' }}
            >
              {/* Desktop table */}
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr style={{ background: '#2D6A4F' }}>
                      <th className="text-left px-5 py-3.5 font-display font-semibold text-white tracking-tight w-[220px]">Term</th>
                      <th className="text-left px-5 py-3.5 font-display font-semibold text-white tracking-tight">Definition</th>
                      <th className="text-left px-5 py-3.5 font-display font-semibold text-white tracking-tight w-[160px]">Category</th>
                    </tr>
                  </thead>
                  <tbody>
                    {filtered.map((term, idx) => (
                      <motion.tr
                        ...
                        className="border-b transition-colors hover:bg-[#f0f7f2]"
                      ...
                      >
                        ...
                        <td className="px-5 py-3.5 align-top leading-relaxed" style={{ color: '#3d5a4c' }}>
                          {term.definition}
                        </td>
                        <td className="px-5 py-3.5 align-top">
                          <span
              ...
              {/* Mobile stacked rows */}
              <div className="sm:hidden divide-y" style={{ borderColor: '#e8f0ec' }}>
                {filtered.map((term, idx) => (
                  <motion.div
                    key={term.term}
                    initial={{ opacity: 0 }}
                  ...
                  >
                    <div className="flex items-start justify-between gap-2">
                      <h3 className="font-display font-bold text-sm" style={{ color: '#2D6A4F' }}>
                        {term.term}
                      </h3>
                      <span
                        className="shrink-0 inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-semibold whitespace-nowrap"
                        style={{
                          background:
                            term.category === 'Company-Specific'
                              ? 'rgba(233, 196, 106, 0.2)'
                    ...
                    <p className="text-sm leading-relaxed" style={{ color: '#3d5a4c' }}>
                      {term.definition}
                    </p>
                  </motion.div>
                ))}
            ...
            >
              ...
              <h3 className="font-display font-semibold text-lg mb-1" style={{ color: '#1B2D2A' }}>
                No matching terms
              </h3>
      ...
      <footer className="border-t py-6" style={{ borderColor: '#e8f0ec', background: '#fff' }}>
        <div className="max-w-[1200px] mx-auto px-4 sm:px-6 flex flex-col sm:flex-row items-center justify-between gap-2">
          <div className="flex items-center gap-2">
            <div
              className="w-6 h-6 rounded flex items-center justify-center font-display font-bold text-xs"
